...
import { isValidUrl, normalizeUrl, isSameDomain, getBaseUrl } from '../utils/url.js';
import { hasSufficientCredits } from '../services/scrapeDoClient.js';
import type { CrawlJobData } from '../queue/types.js';
import 'dotenv/config'

// Store active SSE connections per job
const jobStreamSubscribers = new Map<string, Set<Response>>();

/**
 * Subscribe to real-time job progress via Server-Sent Events
 * GET /api/jobs/:jobId/stream
 */
export async function streamJobProgressHandler(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (!jobStreamSubscribers.has(jobId)) {
      jobStreamSubscribers.set(jobId, new Set());
    }
    jobStreamSubscribers.get(jobId)!.add(res);

    console.log(`[Stream] Client connected to job ${jobId}, total subscribers: ${jobStreamSubscribers.get(jobId)!.size}`);
    ...
    res.on('close', () => {
      ...
      if (subscribers) {
        ...
        console.log(`[Stream] Client disconnected from job ${jobId}, remaining subscribers: ${subscribers.size}`);
        if (subscribers.size === 0) {
          jobStreamSubscribers.delete(jobId);
        }
      }
    });
    ...
    res.status(500).json({
      ...
      message: error instanceof Error ? error.message : 'Failed to establish job stream',
    });
  }
}
 ...
 */
export function broadcastJobProgress(jobId: string, progressData: any): void {
  const subscribers = jobStreamSubscribers.get(jobId);
...
export async function submitJobHandler(req: Request, res: Response): Promise<void> {
  try {
    ...
    const hasCredits = await hasSufficientCredits(scrapeDoToken, 5);
    if (!hasCredits) {
      res.status(402).json({
        error: 'Insufficient scrape.do credits',
        message: 'Your scrape.do account does not have enough remaining monthly requests to process this job',
      });
      return;
    }
...
/**
 * Submit a new connected-pages job to find pages linking to target URL
 * POST /api/jobs/connected-pages/submit
 */
export async function submitConnectedPagesJobHandler(req: Request, res: Response): Promise<void> {
  try {
    const { siteUrl, targetUrl, maxPages, maxDepth, rateLimit } = {
      ...req.query,
      ...req.body,
    };
    ...
    if (!targetUrl || typeof targetUrl !== 'string') {
      res.status(400).json({
        ...
        message: 'Please provide a valid target URL to find inbound links for',
      });
      return;
    }

    if (!isValidUrl(siteUrl) || !isValidUrl(targetUrl)) {
      res.status(400).json({
        error: 'Invalid URL format',
        message: 'siteUrl and targetUrl must be valid HTTP or HTTPS URLs',
      });
      return;
    }

    const normalizedSiteUrl = normalizeUrl(siteUrl, false);
    const normalizedTargetUrl = normalizeUrl(targetUrl, false);

    if (!isSameDomain(normalizedTargetUrl, getBaseUrl(normalizedSiteUrl))) {
      res.status(400).json({
        error: 'Domain mismatch',
        message: 'targetUrl must belong to the same domain as siteUrl',
      });
      return;
    }
    ...
    const job = await crawlQueue.add('crawl', jobData, {
      jobId: `connected-pages-${Date.now()}-${Math.random().toString(36).substring(7)}`,
    });

    console.log(`📥 Connected-pages job ${job.id} submitted for site: ${normalizedSiteUrl}, target: ${normalizedTargetUrl}`);
    ...
    console.error('Error submitting connected-pages job:', error);

    res.status(500).json({
      error: 'Internal server error',
      message: error instanceof Error ? error.message : 'Failed to submit connected-pages job',
    });
  }
}
...
export async function getJobResultHandler(req: Request, res: Response): Promise<void> {
  try {
    ...
    const isConnectedPagesJob = resultSummary.summary?.jobType === 'connected-pages';

    if (isConnectedPagesJob) {
      const resolvedTargetUrl = resultSummary.summary?.targetUrl || '';
      const connectedPages = dbResult.internalLinks
        .filter((link) => resolvedTargetUrl.length > 0 && (link.targetUrls as string[]).includes(resolvedTargetUrl))
      ...
      res.json({
        ...
        data: {
          jobType: 'connected-pages',
          siteUrl: dbResult.targetUrl,
          targetUrl: resolvedTargetUrl,
          connectedPages,
          connectedPagesCount: connectedPages.length,
          targetFound: connectedPages.length > 0,
          metadata: {
            totalPagesChecked: resultSummary.summary?.totalPagesChecked || dbResult.totalPagesCrawled,
            ...
            totalCreditsUsed: dbResult.totalCreditsUsed,
